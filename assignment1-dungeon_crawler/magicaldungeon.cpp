...
Room* MagicalDungeon::createRoom(int i){
    int type = rand() % 2;
    if (type >=1){
        return new EnchantedLibrary(i);
    }
    return new AlchemistsLaboratory(i);
}
core::creatures::Creature* MagicalDungeon::createCreature(bool isBoss){
    int type = rand() % 2;
    if (type >= 1){
        return new core::creatures::Goblin(isBoss);
    }
    return new core::creatures::EvilWizard(isBoss);
}
core::items::Item* MagicalDungeon::createItem(){
    int type = rand() % 4;
    if (type >= 3){
        return new core::items::WizardsStaff();
    }
    else if (type >= 2){
        return new core::items::MagicWand();
    }
    else if (type >= 1){
        return new core::items::Teleporter();
    }
    else{
...
EnchantedLibrary::EnchantedLibrary(int id)
    :Room(id,"The smell of musty pages and the tingle of magic fills the air")
{
...
AlchemistsLaboratory::AlchemistsLaboratory(int id)
    :Room(id,"Your ears are filled with the sounds of numerous elixirs bubbling away over open flams")
...
MagicWall::MagicWall()
    :Wall("A strange barrier blocks your path, almost invisible to the naked eye")
{
